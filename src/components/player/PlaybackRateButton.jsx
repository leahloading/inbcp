...
// array of playback rate objects at 3 speeds (1, 1.5x, 2x) with corresponding icons.
// ie each svg is an icon that matches the corresponding value property on the object e.g 1.5 has an icon with 1.5x => [Tailwind Play](https://play.tailwindcss.com/nOkPac5KVX)
const playbackRates = [
  {
    value: 1,
...
export function PlaybackRateButton({ player }) {
  ...
  // initialise playbackRate state with the 1x speed and icon
  let [playbackRate, setPlaybackRate] = useState(playbackRates[0])


  // the playbackRate button works by selecting the next speed up (you can't just choose the one you want)
  return (
    <button
      type="button"
      ...
      // some logic to select the next playback rate
      onClick={() => {
    ...
    >
      ...
      {/* display the playbackRate icon (ie the icon property from our objects) */}
